...
int main(int argc, char *argv[]) {
    ...
    if (output_mode != 0 && output_mode != 1 && output_mode != 2){
    ...
    if (print_partition != 0 && print_partition != 1){
      printf("Invalid print partition");
      exit(1);
    }

    //this initializes our game board by reading in the file
    ...
    if (output_mode == OUTPUT_VISI) {
       //init_pthread_animation(num_threads, data->rows, data->cols, OUTPUT_VISI, data->iters);
        ...
        handle = init_pthread_animation(num_threads, data.rows, data.cols, visi_name);
        if (handle == NULL) {
    ...
    struct gol_data* arguments = malloc(num_threads * sizeof(struct gol_data));
    if (arguments == NULL) {
      perror("malloc()");
      free(arguments);
      exit(1);
    ...
    secs = end_time.tv_sec*1000000+end_time.tv_usec
     ...
     printf("number of alive cells processed: %d",live);
